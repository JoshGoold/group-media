...
const GroupMembers = ({groupData, groupid, getData}) => {
  ...
  async function denyRequest(username){
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_ROUTE}deny-participant`, {
        groupid: groupid,
        username: username
      }, {
        headers: {
  ...
  async function acceptRequest(username){
    const getToken = localStorage.getItem('token');
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_ROUTE}accept-participant`, {
        groupid: groupid,
        username: username
      }, {
        headers: {
