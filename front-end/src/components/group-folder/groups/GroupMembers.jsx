...
const GroupMembers = ({groupData, groupid, getData}) => {
  ...
  async function denyRequest(username){
    ...
    try {
      const response = await axios.post("https://group-media-4tf1q58sf-joshs-projects-9174c388.vercel.app/deny-participant", {
        groupid: groupid,
        username: username
      }, {
        headers: {
  ...
  async function acceptRequest(username){
    const getToken = localStorage.getItem('token');
    try {
      const response = await axios.post("https://group-media-4tf1q58sf-joshs-projects-9174c388.vercel.app/accept-participant", {
        groupid: groupid,
        username: username
      }, {
        headers: {
